...
class SegmNetSE(nn.Module):
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None):
        ...
        out = self.post2(F.upsample(self.f2(feat_test[2]) + self.se2(self.s2(out)), scale_factor=2))
        out = self.post1(F.upsample(self.f1(feat_test[1]) + self.se1(self.s1(out)), scale_factor=2))
        out = self.post0(F.upsample(self.f0(feat_test[0]) + self.se0(self.s0(out)), scale_factor=2))
