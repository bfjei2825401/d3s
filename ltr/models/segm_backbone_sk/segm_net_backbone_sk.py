...
class SegmNetBackboneSK(nn.Module):
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None):
        ...
        out = self.post1(F.upsample(self.sk1(self.f1(feat_test[1])) + self.s1(out), scale_factor=2))
        out = self.post0(F.upsample(self.sk0(self.f0(feat_test[0])) + self.s0(out), scale_factor=2))
