...
from ltr.models.layers.sk_block import SKConv3x3MeanMaxAdaptive
from ltr.models.layers.common_block import conv, conv_no_relu


class SegmNetSK3x3MeanMaxAdaptive(nn.Module):
    ...
    def __init__(self, segm_input_dim=(128, 256), segm_inter_dim=(256, 256), segm_dim=(64, 64), mixer_channels=2,
        ...
        self.sk2 = SKConv3x3MeanMaxAdaptive(segm_inter_dim[2], M=scale_num)
        self.sk1 = SKConv3x3MeanMaxAdaptive(segm_inter_dim[1], M=scale_num)
        self.sk0 = SKConv3x3MeanMaxAdaptive(segm_inter_dim[0], M=scale_num)
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d) or isinstance(m, nn.Linear):
                ...
                if m.bias is not None:
                    m.bias.data.zero_()
            elif isinstance(m, SKConv3x3MeanMaxAdaptive):
                m.kaiming_normal_initialize()
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None):
        ...
        if test_dist is not None:
            ...
            segm_layers = torch.cat((torch.unsqueeze(pred_sm[:, :, :, 0], dim=1), torch.unsqueeze(pred_pos, dim=1)),
                                    dim=1)

        out = self.mixer(segm_layers)
        out = self.s3(F.upsample(out, scale_factor=2))

        out = self.post2(F.upsample(self.f2(feat_test[2]) + self.sk2(self.s2(out)), scale_factor=2))
        out = self.post1(F.upsample(self.f1(feat_test[1]) + self.sk1(self.s1(out)), scale_factor=2))
        out = self.post0(F.upsample(self.f0(feat_test[0]) + self.sk0(self.s0(out)), scale_factor=2))
