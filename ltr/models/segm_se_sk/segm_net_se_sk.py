...
from ltr.models.layers.se_block import SELayer


class SegmNetSESK(nn.Module):
    ...
    def __init__(self, segm_input_dim=(128, 256), segm_inter_dim=(256, 256), segm_dim=(64, 64), mixer_channels=2,
        ...
        self.se2 = SELayer(segm_inter_dim[2])
        self.se1 = SELayer(segm_inter_dim[1])
        self.se0 = SELayer(segm_inter_dim[0])
    ...
    def forward(self, feat_test, feat_train, mask_train, test_dist=None):
        ...
        out = self.post2(F.upsample(self.f2(feat_test[2]) + self.sk2(self.se2(self.s2(out))), scale_factor=2))
        out = self.post1(F.upsample(self.f1(feat_test[1]) + self.sk1(self.se1(self.s1(out))), scale_factor=2))
        out = self.post0(F.upsample(self.f0(feat_test[0]) + self.sk0(self.se0(self.s0(out))), scale_factor=2))
