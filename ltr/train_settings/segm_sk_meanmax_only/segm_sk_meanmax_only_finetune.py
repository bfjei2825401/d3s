...
import ltr.models.segm_sk_meanmax_only.segm_sk_meanmax_only as models
...
def run(settings):
    ...
    net = models.segm_sk_meanmax_only_resnet50(backbone_pretrained=True, topk_pos=settings.segm_topk_pos,
                                               topk_neg=settings.segm_topk_neg, mixer_channels=mixer_channels)

    net.load_state_dict(torch.load('/home/slz/GitHub/d3s/pytracking/networks/SegmNet.pth.tar')['net'], strict=False)
    # Set objective
    ...
    # Optimizer
    optimizer = optim.Adam(actor.net.segm_predictor.parameters(), lr=5e-4)

    # Learning rate scheduler
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=8, gamma=0.2)
