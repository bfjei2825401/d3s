...
def parameters():
    ...
    if params.use_resnet50:
        # 深度特征滤波器，这里使用 AtomResNet50 作为特征提取器，也就是把 Atom 网络作为一个提取特征的工具，这里只采用了 layer3 的输出作为特征
        # 但实际上这里只用到了 Atom 网络中提取 ResNet 网络特征的功能，没有关于 Atom 网络的部分（IoUNet等），实际上还是提取的 ResNet 网络特征
        # 因为作者把 IoUNet 相关的部分都关掉了（注释）
    ...
    params.segm_net_path = '/home/slz/GitHub/d3s/pytracking/networks/SegmNet.pth.tar'
